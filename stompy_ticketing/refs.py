...
TICKET_REF_RE = re.compile(r"^([A-Za-z][A-Za-z0-9]{0,9})-(\d{1,9})$")
...
def coerce_ticket_ref(
    ...
    Returns ``(current_project, id)`` for int/digit forms, or
    ``(resolved_project, id)`` for prefixed forms. ``(current_project, None)``
    ...
    if _DIGITS_RE.match(ref):
        return current_project, int(ref)
